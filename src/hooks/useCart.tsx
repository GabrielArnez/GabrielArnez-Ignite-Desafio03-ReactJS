...
export function CartProvider({ children }: CartProviderProps): JSX.Element {
  ...
  const addProduct = async (productId: number) => {
    try {
      const productAlreadyCart = cart.find(
        (product) => product.id === productId
      );
      const { data: inStock } = await api.get<UpdateProductAmount>(
        `stock/${productId}`
      );
      ...
      if (!productAlreadyCart) {
        ...
        if (inStock.amount > 0) {
          ...
          toast("Adiciou no carrinho");
          return;
        } else {
          toast.error("fora de estoque");
        }
      } else if (productAlreadyCart) {
        if (inStock.amount > productAlreadyCart.amount) {
